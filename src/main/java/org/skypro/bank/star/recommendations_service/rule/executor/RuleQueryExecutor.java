 ...
 */
public interface RuleQueryExecutor {
     ...
     */
    default void validateArguments(List<String> arguments, int expectedCount) {
        if (arguments == null) {
        ...
        if (arguments.size() != expectedCount) {
            throw new IllegalArgumentException(
                    String.format("Expected %d arguments for query type %s, but got %d: %s",
                            expectedCount, getSupportedQueryType().name(), arguments.size(), arguments)
            );
        }
    }
     ...
     */
    default void validateArgumentsNotEmpty(List<String> arguments) {
        if (arguments == null || arguments.isEmpty()) {
            throw new IllegalArgumentException(
                    String.format("Arguments list cannot be empty for query type %s",
                            getSupportedQueryType().name())
            );
        }
    }
